...
func (p *Postgres) ListRefreshTokensForUser(ctx context.Context, userID string) ([]RefreshToken, error) {
	rows, err := p.db.QueryContext(ctx,
		`SELECT token_hash, user_id, expires_at, created_at, used FROM refresh_tokens
		 WHERE user_id = $1 AND used = FALSE AND expires_at > $2 ORDER BY created_at DESC`, userID, unix(time.Now()))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var rt RefreshToken
		var expires, created int64
		if err := rows.Scan(&rt.TokenHash, &rt.UserID, &expires, &created, &rt.Used); err != nil {
			return nil, err
		}
		rt.ExpiresAt, rt.CreatedAt = fromUnix(expires), fromUnix(created)
		out = append(out, rt)
